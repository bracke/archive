...
ZIP and gzip mutation must use `zlib` for compression boundaries and archive-owned ZIP structures for container metadata. Unsupported write formats should be reported as unavailable commands, not silently downgraded to extraction-only behavior.
