...
ZIP, gzip, bzip2, and Zstandard mutation must use `zlib` for compression boundaries and archive-owned ZIP structures for container metadata where applicable. ZIP BZip2, ZIP LZMA, and ZIP Zstandard payload reads and host-file publication use the zlib ZIP external-method bridge. Rewriting unchanged entries from an existing ZIP through those external methods is not currently exposed; use the stored/Deflate rewrite path unless a staged payload-file compressor is added. 7z mutation must use the public `zlib` Seven_Zip APIs for the supported native subset. Unsupported write formats should be reported as unavailable commands, not silently downgraded to extraction-only behavior.
