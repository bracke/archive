...
package body Archive.Archives.Readers.Zip is
   ...
   function Index_Buffer (Bytes : Zlib.Byte_Array) return Zip_Index_Result is
      Result : Zip_Index_Result;
      EOCD   : constant Natural := Find_EOCD (Bytes);
   begin
      ...
      if EOCD >= 20 and then Signature (Bytes, EOCD - 20) = 16#0706_4B50# then
         Result.Status := Archive.Archives.Errors.Unsupported_Format;
         return Result;
      end if;

      if U16 (Bytes, EOCD + 4) /= 0 or else U16 (Bytes, EOCD + 6) /= 0 then
         Result.Status := Archive.Archives.Errors.Unsupported_Format;
         return Result;
      end if;

      declare
         Entries_On_Disk_Raw : constant Interfaces.Unsigned_16 := U16 (Bytes, EOCD + 8);
         Entries_Total_Raw   : constant Interfaces.Unsigned_16 := U16 (Bytes, EOCD + 10);
         Directory_Size_Raw  : constant Interfaces.Unsigned_32 := U32 (Bytes, EOCD + 12);
         Directory_Off_Raw   : constant Interfaces.Unsigned_32 := U32 (Bytes, EOCD + 16);
         Archive_Comment : constant Natural := Natural (U16 (Bytes, EOCD + 20));
         Entries_On_Disk : Natural;
         Entries_Total   : Natural;
         Directory_Size  : Natural;
         Directory_Off   : Natural;
         Cursor          : Natural;
      begin
         ...
         Entries_On_Disk := Natural (Entries_On_Disk_Raw);
         Entries_Total := Natural (Entries_Total_Raw);
         Directory_Size := Natural (Directory_Size_Raw);
         Directory_Off := Natural (Directory_Off_Raw);
         Cursor := Directory_Off;

         if Entries_On_Disk /= Entries_Total then
            Result.Status := Archive.Archives.Errors.Unsupported_Format;
            return Result;
         elsif not In_Range (Bytes, Directory_Off, Directory_Size) then
            Result.Status := Archive.Archives.Errors.Invalid_Format;
            return Result;
         elsif Directory_Off + Directory_Size /= EOCD then
            Result.Status := Archive.Archives.Errors.Invalid_Format;
            return Result;
         elsif EOCD + EOCD_Min_Size + Archive_Comment /= Bytes'Length then
            Result.Status := Archive.Archives.Errors.Invalid_Format;
            return Result;
         end if;

         for Ordinal in 0 .. Entries_Total - 1 loop
            if not In_Range (Bytes, Cursor, 46)
              or else Signature (Bytes, Cursor) /= 16#0201_4B50#
            then
               Result.Status := Archive.Archives.Errors.Invalid_Format;
               return Result;
            end if;
            ...
            declare
               Flags      : constant Interfaces.Unsigned_16 := U16 (Bytes, Cursor + 8);
               Method     : constant Interfaces.Unsigned_16 := U16 (Bytes, Cursor + 10);
               CRC        : constant Interfaces.Unsigned_32 := U32 (Bytes, Cursor + 16);
               Comp_Size_Raw : constant Interfaces.Unsigned_32 := U32 (Bytes, Cursor + 20);
               Uncomp_Raw    : constant Interfaces.Unsigned_32 := U32 (Bytes, Cursor + 24);
               Name_Len   : constant Natural := Natural (U16 (Bytes, Cursor + 28));
               Extra_Len  : constant Natural := Natural (U16 (Bytes, Cursor + 30));
               Comment_Len : constant Natural := Natural (U16 (Bytes, Cursor + 32));
            ...
            begin
               ...
               declare
                  Name : constant String := Name_At (Bytes, Header_End, Name_Len);
                  ...
                  Unicode_Name : constant Unicode_Path_Result :=
                    Unicode_Path_From_Extra (Bytes, Extra_Offset, Extra_Len, Name);
                  ...
                  Local_Name_Len  : Natural := 0;
                  Local_Extra_Len : Natural := 0;
               begin
                  ...
                  Comp_Size := Natural (Comp_Size_64);
                  Uncomp := Natural (Uncomp_64);
                  Local_Off := Natural (Local_Off_64);

                  if not In_Range (Bytes, Local_Off, 30)
                    or else Signature (Bytes, Local_Off) /= 16#0403_4B50#
                    or else U16 (Bytes, Local_Off + 6) /= Flags
                    or else U16 (Bytes, Local_Off + 8) /= Method
                  then
                     Result.Status := Archive.Archives.Errors.Invalid_Format;
                     return Result;
                  end if;
                  ...
                  declare
                     ...
                     Local_Comp_Size      : constant Interfaces.Unsigned_32 := U32 (Bytes, Local_Off + 18);
                     Local_Uncomp_Size    : constant Interfaces.Unsigned_32 := U32 (Bytes, Local_Off + 22);
                     Local_ZIP64          : constant ZIP64_Extra_Result :=
                       ZIP64_From_Extra
                         (Bytes,
                          Local_Off + 30 + Local_Name_Len,
                          Local_Extra_Len,
                  ...
                  begin
                     ...
                     then
                        Result.Status := Archive.Archives.Errors.Invalid_Format;
                        return Result;
                     elsif not In_Range (Bytes, Data_Start, Comp_Size) then
                        Result.Status := Archive.Archives.Errors.Invalid_Format;
                        return Result;
                     elsif Descriptor_Start > Directory_Off then
                        Result.Status := Archive.Archives.Errors.Invalid_Format;
                        return Result;
                     elsif Uses_Data_Descriptor then
                        Descriptor :=
                          Descriptor_At
                            (Bytes, Descriptor_Start, CRC, Comp_Size_64, Uncomp_64);
                       ...
                       To_Unbounded_String
                         (ZIP_Metadata
                            (Flags, Extra_Len, Comment_Len, Unicode_Name.Present,
                             (Flags and 2048) /= 0, ZIP64.Present or else Local_ZIP64.Present,
                  ...
                  Item.Display_Name :=
                    To_Unbounded_String
                      (Archive.Archives.Paths.Safe_Display_Name
                         ((if Unicode_Name.Present then To_String (Unicode_Name.Path) else Name)));
                  if Comment_Len > 0 then
                     Item.Comment := To_Unbounded_String
                       (Name_At (Bytes, Comment_Offset, Comment_Len));
                  end if;
                  Item.Data_Offset :=
                    (Present => True,
                     Value => Archive.Types.Source_Offset (Local_Off + 30 + Local_Name_Len + Local_Extra_Len));
         ...
         if Cursor /= Directory_Off + Directory_Size then
            Result.Status := Archive.Archives.Errors.Invalid_Format;
         end if;
      end;
