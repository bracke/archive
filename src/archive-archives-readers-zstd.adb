...
package body Archive.Archives.Readers.Zstd is
   ...
   function Logical_Name (Source_Name : String) return String is
      Name : constant String := (if Source_Name'Length > 0 then Source_Name else "payload.zst");
   begin
      if Has_Suffix (Name, ".zstd") and then Name'Length > 5 then
         declare
            Candidate : constant String := Name (Name'First .. Name'Last - 5);
         begin
            if Safe_Name (Candidate) /= "" then
               return Candidate;
            end if;
         end;
      elsif Has_Suffix (Name, ".zst") and then Name'Length > 4 then
         declare
   ...
   begin
      ...
      Result.Item.Method := Archive.Archives.Entries.Zstd_Compression;
...
end Archive.Archives.Readers.Zstd;
